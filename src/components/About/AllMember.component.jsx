...
export const AllMember =() =>{
    ...
    return(
        ...
        <Container className="grid lg:grid-cols-5 grid-cols-2 sm:w-8/12 w-full">
            {
                Members.slice(4).map((member,index)=><Member last={index === Members.slice(4).length - 1} key={index} {...member}/>)
            }
        </Container>
